...
module "blue_container" {
  ...
  image     = "374269020027.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/${local.app_name}-ecr-repo:${var.blue_image_tag}"
  essential = true
...
module "blue_deploy" {
  ...
  service_load_balancers = [
    {
      target_group_arn = data.aws_lb_target_group.blue_target_group.arn
...
module "green_container" {
  count = var.green_image_tag != "" ? 1 : 0

  source    = "git::https://github.com/ministryofjustice/modernisation-platform-terraform-ecs-cluster//container?ref=b00647922a9204a99b023ac884440162e2b51b66" # v4.3.0
  name      = "${local.container_name}-green"
  image     = "374269020027.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/${local.app_name}-ecr-repo:${var.green_image_tag}"
  essential = true
...
module "green_deploy" {
  ...
  service_load_balancers = [
    {
      target_group_arn = data.aws_lb_target_group.green_target_group.arn
