...
output "ecs_service_arn_blue" {
  value = var.sub_env == "sandbox" && var.blue_image_tag != "" ? module.blue_deploy[0].service_arn : null
}

output "ecs_service_arn_green" {
  value = var.sub_env == "sandbox" && var.green_image_tag != "" ? module.green_deploy[0].service_arn : null
}
